#ifndef __ZEGOSYNCALL_INC__
#define __ZEGOSYNCALL_INC__
...
#define ZEGO_TASK_CALL_API(T,F, ...)		\
typedef struct zego_task_call_##T##F : public T::zego_task_call_base_##T {FIELD(__VA_ARGS__);}zego_task_call_##T##F;\
static inline boolean F##_hidden(T::zego_task_call_base_##T* base) {\
	zego_task_call_##T##F* __p__ = (zego_task_call_##T##F*)base;\
	boolean r = __p__->fthis->F(EXPAND(__VA_ARGS__));\
    CZEGOEvent* pevent = (CZEGOEvent*)__p__->_event;\
    if(pevent) pevent->Post();\
    return r;\
}

#define ZEGO_TASK_CALL_API_NR(T,F, ...)		\
    typedef struct zego_task_call_##T##F : public T::zego_task_call_base_##T {FIELD(__VA_ARGS__);}zego_task_call_##T##F;\
    static inline void F##_hidden(T::zego_task_call_base_##T* base) {\
    zego_task_call_##T##F* __p__ = (zego_task_call_##T##F*)base;\
...
#define ZEGO_TASK_CALL_HERE(TASK,_T,F,...) \
    if( TASK->getthreadid() != zegothread_selfid() )\
    {\
        zego_task_call_##_T##F *__p__ = new zego_task_call_##_T##F;\
        __p__->fname = #F;\
        __p__->func_nr = (zego_any_call_nr)F##_hidden;\
        __p__->fthis = this;\
        __p__->_event = NULL;\
        CScopePtr<CCallTaskArg> arg(eDoNew);\
        arg->pCall = __p__;\
        CScopeCall call(this,&_T::zego_asyn_call,(CCallTaskArg*)arg); \
        ASSIGN(__VA_ARGS__);\
        CZEGOTaskDefault::PushTask(TASK,call);\
        return true;\
    }

#define ZEGO_TASK_CALL_HERE_NR(TASK,_T,F,...) \
    if( TASK && TASK->getthreadid() != zegothread_selfid() )\
    {\
        zego_task_call_##_T##F *__p__ = new zego_task_call_##_T##F;\
        __p__->fname = #F;\
        __p__->func_nr = (zego_any_call_nr)F##_hidden;\
...
#define ZEGO_TASK_SYNC_CALL_HERE(TASK,_T,F,...) \
    if( TASK && TASK->getthreadid() != zegothread_selfid() )\
    {\
        zego_task_call_##_T##F *__p__ = new zego_task_call_##_T##F(__VA_ARGS__);\
        __p__->fname = #F;\
        __p__->func_nr = (zego_any_call_nr)F##_hidden;\
        __p__->fthis = this;\
        __p__->event = (void*)(new CZEGOEvent());\
        CScopePtr<CCallTaskArg> arg(eDoNew);\
        arg->pCall = __p__;\
        CScopeCall call(this,&_T::zego_asyn_call,(CCallTaskArg*)arg); \
        ASSIGN(__VA_ARGS__);\
...
#define ZEGO_TASK_SYNC_CALL_HERE_NR(TASK,_T,F,...) \
    if( TASK && TASK->getthreadid() != zegothread_selfid() )\
    {\
        zego_task_call_##_T##F *__p__ = new zego_task_call_##_T##F;\
        __p__->fname = #F;\
        __p__->func_nr = (zego_any_call_nr)F##_hidden;\
        __p__->fthis = this;\
        __p__->_event = (void*)(new CZEGOEvent());\
        CScopePtr<CCallTaskArg> arg(eDoNew);\
        arg->pCall = __p__;\
        CScopeCall call(this,&_T::zego_asyn_call,(CCallTaskArg*)arg); \
        ASSIGN(__VA_ARGS__);\
...
#define ZEGO_TASK_CALL_NEXT_CPU_TIME(_T,F,...) {\
    zego_task_call_##_T##F *__p__ = new zego_task_call_##_T##F(__VA_ARGS__);\
    __p__->fname = #F;\
    __p__->func_nr = (zego_any_call_nr)F##_hidden;\
    __p__->fthis = this;\
    ...
    CScopeCall call(this,&_T::zego_asyn_call,(CCallTaskArg*)arg); \
    ASSIGN(__VA_ARGS__);\
    CZEGOTaskDefault::PushTask(call);\
    }

#define ZEGO_TASK_CALL_WHERE(TASK,_T,F,...) {\
    zego_task_call_##_T##F *__p__ = new zego_task_call_##_T##F;\
    __p__->fname = #F;\
    __p__->func_nr = (zego_any_call_nr)F##_hidden;\
    __p__->fthis = this;\
